...
module ActiveAdmin
  ...
  class Resource   
      ...
      def links?(context = nil)
        ...
        # return render_in_context(context, links_config[:if]) unless links_config[:if].nil?
        # return !render_in_context(context, links_config[:unless]) unless links_config[:unless].nil?
        true
      end
  end#Resource
