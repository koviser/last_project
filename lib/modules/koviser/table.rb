  ...
  module Views 
    ...
    class TableFor < Arbre::HTML::Table
      ...
      def column(*args, &block)
        elem = args.first
        if @config
          link = @config.links? && elem.is_a?(Symbol) && block.is_a?(NilClass) && (@config.links_method == :all || @config.links_only.include?(elem) || @config.links_methods.include?(elem)) && !@config.links_expect.include?(elem) if @config
        else
